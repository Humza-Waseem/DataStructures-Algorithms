...
def asc_bucket_sort_data(datas, col):
    start = time.time()
    data = copy.copy(datas)
    req = getattr(data, col)
    min_val = min(req)
...
def asc_counting_sort(data, col):
    start = time.time()
    req = getattr(data, col)
    max_val = max(req)
    count = [0] * (max_val + 1)
...
def counting_sort(data, col, digit, base):
    req = getattr(data, col)
    max_val = max(req)
    ...
    # Update count to store the last index for each digit
    ...
    # Build the output array using count information
    for i in range(len(req) - 1, -1, -1):
        digit_value = (req[i] // (base ** digit)) % base
        output[count[digit_value] - 1] = i
        count[digit_value] -= 1

    # Populate the sorted data using the output array
    for i in output:
        sorted_data.title.append(data.title[i])
        sorted_data.answers.append(data.answers[i])
        sorted_data.views.append(data.views[i])
...
def asc_radix_sort(data, col, base=10):
    start = time.time()
    ...
    for digit in range(num_digits):
        sorted_data = counting_sort(data, col, digit, base)
    end = time.time()
    return sorted_data, end - start
